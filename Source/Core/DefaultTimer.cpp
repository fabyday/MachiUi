...
void DefaultTimer::tick() {
  ...
  std::chrono::duration<double> frameDiff = currentTime - _lastTickTime;
  ...
  if (_isPaused) {
    ...
    std::chrono::duration<double> deltaSec = frameDiff;
...
uint64_t DefaultTimer::getTotalTimeMS() { return this->totalMS; }
